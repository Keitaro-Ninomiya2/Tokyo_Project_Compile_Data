...
def extract_names(text):
    ...
    Returns list of dicts: [{'name': '山田太郎', 'method': 'sudachi'}, ...]
    """
    ...
    for token in tokens:
        ...
        if pos[0] == '名詞' and pos[1] == '固有名詞' and pos[2] == '人名':
            if pos[3] == '姓':
                # New surname encountered — flush any pending incomplete name
                if temp_name:
                ...
                # Orphaned given name (rare) — keep as standalone
                names.append(token.surface())
            else:
            ...
            if temp_name:
                names.append(''.join(temp_name))
                temp_name = []
            # Skip non-name tokens (these should already be stripped,
            # but this handles any residual particles/symbols)
