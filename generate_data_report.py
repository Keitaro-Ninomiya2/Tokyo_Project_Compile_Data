...
def df_to_md(df):
    """Convert a DataFrame to a markdown table without tabulate dependency."""
...
def main():
    ...
    if 'is_name' in new.columns:
        ...
        if len(non_name) > 0:
            ...
            lines.append(sample[sample_cols].pipe(df_to_md))
    else:
    ...
    if len(clean) > 0:
        lines.append(clean[display_cols].pipe(df_to_md))
    else:
