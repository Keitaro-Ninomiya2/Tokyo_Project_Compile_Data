...
print(f'Applying V3 patch for {path}...')
...
for line in lines:
    # 1. Remove ANY previous patch versions to ensure a clean slate
    if '# PATCH:' in line:
        skip_mode = True
    ...
    # 2. Insert the V3 patch
    if 'self.model = init_detector' in line and not skip_mode:
        indent = '        ' # 8 spaces
        new_lines.append(f'{indent}# PATCH: Fix MMDetection 3.x Config Issues (V3)\n')
        new_lines.append(f'{indent}if hasattr(self.model, "cfg"):\n')
        new_lines.append(f'{indent}    # Fix A: Remove deprecated args and inject valid transforms\n')
        ...
        new_lines.append(f'{indent}                        dict(type="Resize", keep_ratio=True, scale=(1024, 1024)),\n')
...
print('✅ Success: Patch V3 applied.')
