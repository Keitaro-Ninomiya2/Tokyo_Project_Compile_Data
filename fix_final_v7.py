...
print(f'Applying V7 (Corrected Pipeline) patch for {path}...')
...
for line in lines:
    # 1. Clean out ANY previous patches (V1-V6) to ensure a clean slate
    if '# PATCH:' in line:
        skip_mode = True
    ...
    # 2. Insert V7 Patch (The Working Simplifier)
    if 'self.model = init_detector' in line and not skip_mode:
        indent = '        ' # 8 spaces
        new_lines.append(f'{indent}# PATCH: Force Simple Pipeline for MMDetection 3.x (V7)\n')
...
print('✅ Success: Patch V7 applied.')
