...
print(f'Applying V11 (Manual GPU Override) patch for {path}...')
...
for line in lines:
    ...
    if 'self.model = init_detector' in line and not skip_mode:
        indent = '        ' # 8 spaces
        new_lines.append(f'{indent}# PATCH: Manual GPU Override for MMDetection 3.x (V11)\n')
        ...
        new_lines.append(f'{indent}        \n')
        new_lines.append(f'{indent}        # 2. MANUAL PREPROCESSOR (No Super() calls, just raw force)\n')
        ...
        new_lines.append(f'{indent}        \n')
        new_lines.append(f'{indent}        # Inject the new brain\n')
        new_lines.append(f'{indent}        self.model.data_preprocessor = NuclearPreprocessor(device="cuda")\n')
        new_lines.append(f'{indent}        \n')
...
print('✅ Success: Patch V11 applied.')
